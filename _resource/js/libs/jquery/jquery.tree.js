...
(function ( $ ) {
    ...
    $.fn.treeview = function ( settings ) {
        var dfop =
        {
             ...
             */
            theme: 'bbit-tree-no-lines', //"bbit-tree-arrows" //bbit-tree-lines" //bbit-tree-arrows,bbit-tree-no-lines,"
        ...
        function buildnode( nd, ht, deep, path, isend ) {
            ...
            var cs = [];
            var ndForCheckType = nd.text.toLowerCase();
            ...
            if ( nd.hasChildren ) {
                ...
                try {
                    if ( _(ndForCheckType).endsWith('.js') ) {
                        cs.push("bbit-tree-node-leaf-js");
                    } else if ( _(ndForCheckType).endsWith('.css') ) {
                        cs.push("bbit-tree-node-leaf-css");
                    } else if ( _(ndForCheckType).endsWith('.html') ) {
                        cs.push("bbit-tree-node-leaf-html");
                    } else if ( _(ndForCheckType).endsWith('.gif') || _(ndForCheckType).endsWith('.png') || _(ndForCheckType).endsWith('.jpg') || _(ndForCheckType).endsWith('.jpeg') ) {
                        cs.push("bbit-tree-node-leaf-img");
                    } else {
            ...
            //a
            ht.push("<a hideFocus class='bbit-tree-node-anchor' tabIndex=1 href='javascript:void(0);' data='" + nd.value + "'>");
            ht.push("<span unselectable='on'>", nd.text);

            if ( nd.dateModified ) {
                ht.push('&nbsp;&nbsp;<span style="color:#999">[ ', nd.dateModified);
                if ( nd.size ) {
                    ht.push(' ~', nd.size);
                }
                ht.push(' ]<\/span>');
            ...
            if ( dfop.showmin && (_(nd.text).endsWith('.js') || _(nd.text).endsWith('.css') ) ) {
                ht.push("<span class='min'></span>");
            }
...
//                            item.ChildNodes != null && asnybuild(item.ChildNodes, deep, path, ul, item);
                            //使目录支持懒加载
                            if ( !item.ChildNodes ) {
                                item.ChildNodes = S_Utils.folder.createFolderList(item.value, item.checkstate);
                            }
        ...
        function InitEvent( parent ) {
            ...
            var anchors = nodes.find('a.bbit-tree-node-anchor');
            anchors.each(function () {
                $(this).dblclick(anchorEventHandler);
            });
        }

        function anchorEventHandler( e ) {
        ...
        function getClickedValue( nid, childNodes ) {
            if ( !childNodes ) {
                var roodId = id + "_" + treenodes[0].id.replace(/[^\w]/gi, "_");
                if ( roodId == nid ) {
                    S_Utils.scriptShell.showInExplorer(treenodes[0].value);
                    return;
                }
                childNodes = treenodes[0]['ChildNodes'];
            }

            if ( childNodes.length > 0 ) {
                for ( var i = 0, length = childNodes.length; i < length; i++ ) {
                    var nnid = id + "_" + childNodes[i].id.replace(/[^\w]/gi, "_");
//                    console.log('nnid:' + nnid);
                    if ( nnid == nid ) {
//                        console.log('found it!!!!!' + childNodes[i].value);
                        S_Utils.scriptShell.showInExplorer(childNodes[i].value);
                        return;
                    }

                    if ( childNodes[i]['ChildNodes'] ) getClickedValue(nid, childNodes[i]['ChildNodes']);
